...
public interface UjianMapelRepository extends JpaRepository<UjianMapel, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT u FROM UjianMapel u WHERE u.eventUjian.id = :eventId AND (" +
           "EXISTS (SELECT 1 FROM u.kelasList k WHERE k.id = (SELECT s.kelas.id FROM com.baknusbelajar.api.entity.Siswa s WHERE s.id = :siswaId)) OR " +
           "(u.kelasList IS EMPTY AND (" +
           "  UPPER(u.eventUjian.namaEvent) LIKE '%LATIHAN%' OR UPPER(u.eventUjian.namaEvent) LIKE '%SIMULASI%' OR u.durasi = 0 OR " +
